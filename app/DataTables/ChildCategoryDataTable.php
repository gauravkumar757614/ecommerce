...
class ChildCategoryDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                // Edit button
                $editBtn       =   "<a href='" . route('admin.child-category.edit', $query->id) . "' class='btn btn-primary'>
                <i class='far fa-edit'></i></a>";
                // Delete button
                $deleteBtn     =   "<a href='" . route('admin.child-category.destroy', $query->id) . "' class='btn btn-danger ml-2 delete-item'>
                 <i class='fas fa-user-times'></i></a>";

                return $editBtn . $deleteBtn;
            })
            // Retrieving name of the parent category through category relation method
            ->addColumn('category', function ($query) {
                return $query->category->name;
            })

            // Retrieving name of the sub category through category relation method
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('sub_category')->width(300),
