    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                // Delete button
                $deleteBtn     =   "<a href='" . route('admin.blog-comment.destroy', $query->id) . "' class='btn btn-danger ml-2 delete-item'>
         <i class='fas fa-user-times'></i></a>";
            ...
            ->addColumn('post', function ($query) {
                return  "<a href='" . route('blog-details', $query->blog->slug) . "'' target='_gaurav'>
                " . $query->blog->title . "
                </a>";
            })
            ...
            ->rawColumns(['action', 'post', 'user_name', 'user_comment'])
            ->setRowId('id');
    }
